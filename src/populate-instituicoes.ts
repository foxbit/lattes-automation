/**
 * Cadastra instituições novas (Pipa, Platform Builders, Leany) + preenche vínculos
 * 
 * Fluxo: abrir form atuação → lupa → buscar → "Cadastrar nova instituição" →
 * f_nme_inst + f_sigla + f_pais_inst=BRA + f_uf → Confirmar → preencher vínculo → Salvar
 * 
 * Uso: npx tsx src/populate-instituicoes.ts [--dry-run]
 */
...
interface Vinculo {
  nomeInst: string;          // nome completo para cadastro
  sigla: string;
  uf: string;
  busca: string;             // termo de busca (deve retornar vazio para forçar cadastro)
...
async function registerNewInstitution(page: Page, formFrame: Frame, v: Vinculo): Promise<boolean> {
  ...
  // Buscar — termo que NÃO existe (para forçar "Cadastrar nova instituição")
  ...
  if (hasResult) {
    ...
    if (clicked.clicked) {
      ...
      // Fechar modal
      const fechar = await cv3Frame.$('input[value="Fechar"], a:has-text("Fechar")');
  ...
  try {
    ...
    console.log(`   ⚠️ País: ${(e as Error).message}`);
  }
  ...
  try {
    ...
    console.log(`   ⚠️ UF: ${(e as Error).message}`);
  }
  await page.waitForTimeout(500);
...
async function main() {
  ...
  console.log(`🏢 Cadastro de instituições + vínculos (${dryRun ? 'DRY-RUN' : 'PRODUÇÃO'})\n`);
  ...
  if (dryRun) {
    console.log('🔒 [DRY-RUN] Seriam cadastrados:');
  ...
  for (const v of VINCULOS) {
    ...
    try {
      ...
      if (!instOk) {
        console.log('   ❌ Falha no cadastro da instituição');
        failed++;
      ...
      if (!instVal) {
        console.log('   ❌ Instituição não preenchida');
        failed++;
        await nav.closeModal();
        await page.waitForTimeout(1000);
        continue;
      ...
      console.log(`   ${vincResult.success ? '✅' : '❌'} Vínculo: ${v.vinculo}`);
